...
We bring an example session in interacting with typical Symbolfront user interface. The client does not contain knowledge of any application commands. Instead, during runtime, the current application context provides a set of commands available in that context. In other words, the application takes care of specific functionality regarding the context.
